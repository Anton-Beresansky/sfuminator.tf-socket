...
function Database(options) {
    this.log = new Logs({applicationName: "Database"});
...
Database.prototype.connect = function (callback) {
    ...
    this.pool.getConnection(function (err, connection) {
        if (err) {
            ...
            dbConnection.on("connectionDecayTimeout", function () {
                self.log.warning("Connection hasn't be released after " + parseInt(self.connectionDecay / 1000) + "s, maybe query is taking too much time?");
...
function DatabaseConnection(connection) {
    ...
    this.log = new Logs({applicationName: "Database (#" + self.connectionID + ")"});
    this.log.disableDebug();
    this.connectionDecay = 1;
    this.connectionDecayTimeout = setTimeout(function () {
        self.emit("connectionDecayTimeout");
