...
function SteamClient(steamid) {
    ...
    this.itemsInEscrowFetchInterval = ***REMOVED***00;
...
SteamClient.prototype.fetchItemsInEscrow = function (callback) {
    var i, self = this;
    this.tradeOffersManager.getOffers(SteamTradeOffersManager.EOfferFilter.All, new Date(0), function (err, sent, received) {
        if (err) {
            self.log.warning("Wasn't able to fetch items in escrow");
            return;
        }
