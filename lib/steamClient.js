    ...
    this.community.on('newConfirmation', function (confirmation) {
        ...
        confirmation.respond(self._getUnixTimestamp(), self.credentials.getConfirmationKey("allow"), true, function (error) {
            if (error) {
                ...
                self._bypassTradeOfferMangerPolling(confirmation.offerID, SteamTradeOffer.SteamTradeStatus.Active);
            }
        });
    });
...
SteamClient.prototype.getOldestFriend = function (whiteList) {
    if (this.getNumberOfFriends() > 0) {
        var oldestFriend = this.friendList[0];
        for (var i = 0; i < this.getNumberOfFriends(); i += 1) {
            if (this.friendList[i].getFriendSince() < oldestFriend.getFriendSince()) {
                if (whiteList && whiteList instanceof Array) {
                    var whiteListed = false;
                    for (var j = 0; j < whiteList.length; j += 1) {
                        if (whiteList[j] === this.friendList[i]) {
                            whiteListed = true;
                            break;
                        }
                    }
...
SteamClient.prototype.postProfileComment = function (steamid, comment, callback) {
    var options = {comment: comment, count: 6, sessionid: this.webSessionID};
    this.steamClient.browser.post("http://steamcommunity.com/comment/Profile/post/" + steamid + "/-1/", options, function (result) {
        if (typeof callback === "function") {
...
SteamClient.prototype._bypassTradeOfferMangerPolling = function (tradeOfferID, newState) {
    var actions = ["sent", "received"];
    for (var i = 0; i < actions.length; i += 1) {
        var action = actions[i];
        if (this.tradeOffersManager.pollData[action].hasOwnProperty(tradeOfferID)) {
            var oldState = this.tradeOffersManager.pollData[action][tradeOfferID];
            //Prevent library from emitting the offerChanged event
            this.tradeOffersManager.pollData[action][tradeOfferID] = newState;
            //Manually emit event
            this.log.debug("Bypassing " + tradeOfferID + " from " + oldState + " to " + newState);
            this._bypassTradeOfferManagerPolling_AntiCallback(action, tradeOfferID, oldState);
        }
    }
};
