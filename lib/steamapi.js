// Sfuminator.tf | Interfaces for Steam Web API
...
SteamAPI.prototype.getSchema = function (callback) {
    ...
    steamAPI.callAPI(myInterface1, function (response1) {
        steamAPI.callAPI(myInterface2, function (response2) {
            for (var prop in response2.result) {
                response1.result[prop] = response2.result[prop];
            }
            callback(response1);
        });
    });
