// Sfuminator.tf | Steam Trade Offer library
...
/**
 * TODO It seems that sometimes states don't fire (happened that 'accepted' (3) didn't fire and trade remained stuck)
 *
 */
...
function SteamTradeOffer(steamClient, partnerSteamID) {
    ...
    this._cancelHasOnceListener = false;
    this._offerIDsStack = [];
    this.timeoutList = [];
...
SteamTradeOffer.prototype.isMaking = function () {
    return this.making_flag;
};
...
SteamTradeOffer.prototype.cancel = function (callback) {
    var self = this;
    this.pauseAutoRetry();
    this.clearTimeouts(); //Blocking any error fix attempts and shit like that
    this.log.test("Cancelling state " + this.state);
    if (this.state === SteamTradeOffer.SteamTradeStatus.Expired ||
        this.state === SteamTradeOffer.SteamTradeStatus.Declined ||
        this.state === SteamTradeOffer.SteamTradeStatus.Canceled ||
        this.state === SteamTradeOffer.SteamTradeStatus.CanceledBySecondFactor) {
        callback();
    } else if (this.getTradeOfferID() || this.isMaking()) {
        if (!this._cancelHasOnceListener) {
            self._cancelHasOnceListener = true;
            self.once('cancelled', function () {
                clearTimeout(self.cancelCheckTimeout);
                callback();
            });
        } else {
            self.log.debug("Cancel requested but listener already placed. Retrying cancel procedure");
        }
        this.cancelTrade();
        clearTimeout(self.cancelCheckTimeout);
        self.cancelCheckTimeout = setTimeout(function () {
            self.loadOffer(function () {
                if (self.state === SteamTradeOffer.SteamTradeStatus.Canceled ||
                    self.state === SteamTradeOffer.SteamTradeStatus.InvalidItems ||
                    self.state === SteamTradeOffer.SteamTradeStatus.Countered ||
                    self.state === SteamTradeOffer.SteamTradeStatus.CanceledBySecondFactor) {
                    self.log.warning("CCTimeout triggered. State is inactive (" + self.state + "). Forcing callback");
                    callback();
                } else {
                    self.log.warning("CCTimeout triggered. State is NOT canceled but " + self.state + ". Doing nothing");
                }
            });
        }, ***REMOVED***0); // 30 Seconds
    } else {
        callback();
    }
...
SteamTradeOffer.prototype.clearTimeouts = function () {
...
SteamTradeOffer.prototype.solveError = function () {
    ...
    } else if (errorCode === SteamTradeError.ERROR.BUSY) {
        this.pauseAutoRetry();
        this._checkIfTradeHasBeenActuallyCreatedAndFix(function (offer) {
            if (offer) {
                self.stopAndResetMaking();
...
SteamTradeOffer.prototype._onTradeOfferCreated = function (tradeOfferID, offer) {
    ...
    this.state = (offer && offer.hasOwnProperty("state")) ? offer.state : this.state;
    this.offer = offer ? offer : this.offer;
...
SteamTradeOffer.prototype.loadOffer = function (callback) {
    var self = this;
    this.steamClient.tradeOffersManager.getOffer(this.getTradeOfferID(), function (error, offer) {
        if (error) {
            self.log.warning("Caution, wasn't able to fetch trade: " + error);
        }
        self.state = (offer && offer.hasOwnProperty("state")) ? offer.state : self.state;
        self.offer = offer ? offer : self.offer;
...
SteamTradeOffer.prototype._emitTradeState = function () {
    ...
    if (this.state === SteamTradeOffer.SteamTradeStatus.Canceled) {
        this.stopAndResetMaking();
        this.emit("cancelled");
    } else if (this.state === SteamTradeOffer.SteamTradeStatus.Accepted) {
        this.stopAndResetMaking();
        this.emit("partnerAccepted", false);
    } else if (this.state === SteamTradeOffer.SteamTradeStatus.Declined || this.state === SteamTradeOffer.SteamTradeStatus.CanceledBySecondFactor) {
        this.stopAndResetMaking();
    ...
    } else if (this.state === SteamTradeOffer.SteamTradeStatus.InvalidItems) {
