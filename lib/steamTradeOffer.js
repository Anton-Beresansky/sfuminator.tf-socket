            ...
            self.steamClient.tradeOffers.makeOffer(self.getOptions(), function (error, tradeResult) {
                ...
                if (error && self.numberOfRetries <= SteamTradeOffer.MAX_MAKE_RETRIES) {
                    ...
                    if (error) {
                        ...
                        self._offerIDsStack.push(self.tradeOfferID);
                        self.steamClient.tradeOffersManager.getOffer(self.tradeOfferID, function (error, offer) {
                            if (error) {
...
SteamTradeOffer.prototype.cancel = function (callback) {
    ...
    /*//It can happen that this trade is associated with more steam offers
     if (this._offerIDsStack.length > 1) {
     //Then we better cancel all the other offers as well
     for (var i = 0; i < this._offerIDsStack - 1; i += 1) {
     this.steamClient.tradeOffers.cancelOffer({tradeOfferId: this._offerIDsStack[i]});
     }
     }*/
};
...
SteamTradeOffer.prototype.solveError = function () {
    ...
    if (errorCode === SteamTradeError.ERROR.ITEMS_REVOKED) {
        ...
        this.pauseAutoRetry();
        this._checkIfTradeHasBeenActuallySentAndFix(function (success) {
            if (!success) {
...
SteamTradeOffer.prototype._startListening = function () {
    ...
    this.steamClient.onTradeOfferChange(this.getTradeOfferID(), function (tradeOffer) {
        ...
        if (self.state === SteamTradeOffer.SteamTradeStatus.Canceled || self.state === SteamTradeOffer.SteamTradeStatus.CanceledBySecondFactor) {
            self.stopAFKCheck();
            self.emit("cancelled");
        } else if (self.state === SteamTradeOffer.SteamTradeStatus.Accepted) {
            self.stopAFKCheck();
            self.emit("partnerAccepted");
        } else if (self.state === SteamTradeOffer.SteamTradeStatus.Declined) {
            self.stopAFKCheck();
            self.emit("partnerDeclined");
        } else if (self.state === SteamTradeOffer.SteamTradeStatus.Active) {
...
SteamTradeOffer.prototype._checkIfTradeHasBeenActuallySentAndFix = function (callback) {
    var self = this;
    setTimeout(function () {
        //Get active offers.
        self.steamClient.tradeOffersManager.getOffers(1, null, function (err, sentOffers) {
            ...
            for (var i = 0; i < sentOffers.length; i += 1) {
                ...
                if (offer.partner.partner.toString() === self.partnerSteamID) {
                    self.log.test("Found sent offer, is it right?");
                    if (self._areSameItems(self.itemsFromMe, offer.items_to_give) && self._areSameItems(self.itemsFromThem, offer.items_to_receive)) {
                        self.log.test("Yes, it is the same, let's fix it");
                        self.tradeOfferID = offer.id;
                        self.state = offer.state;
                        fixed = true;
                    }
                }
