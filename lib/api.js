...
APIs.prototype.raw_callAPI = function(myInterface, callback) {
    ...
    if (myInterface.method.hasOwnProperty("httpmethod") && myInterface.method.httpmethod === "POST") {
        ...
        var path = stringinterfacename + "/" + myInterface.method.name + stringversion + predata;
        ...
        var post_req = http.request(post_options, function(res) {
            ...
            res.on('end', function() {
                ...
                try {
                    ...
                    finalResult = {result: "success", content: body, options: post_options};
                } catch (e) {
        ...
        var path = stringinterfacename + "/" + myInterface.method.name + stringversion + "/" + predata + "?" + data;
        var options = {host: base_url, path: path, headers: {Cookie: cookies}};
...
APIs.prototype.loadPage = function(url, callback) {
    var http = require("http");
    for (var x in url) {
        if (url[x] === "/") {
            var base_url = url.slice(0, x);
            var path = url.slice(x, url.length);
            break;
        }
    }
    var cookies = "";
    ...
    http.get(options, function(res) {
        ...
        res.on('end', function() {
            ...
            try {
                var body = Buffer.concat(bodyChunks);
                finalResult = body.toString("utf8");
            } catch (e) {
                finalResult = {result: "fail", type: "error", message: "error when fetching data", error: e, options: options};
            }
            if (callback) {
                callback(finalResult);
            }
        ...
        res.on('error', function(e) {
            if (callback) {
                callback({result: "fail", type: "error", message: "error when loading page", error: e, options: options});
            }
        });
    }).on('error', function(e) {
        if (callback) {
            callback({result: "fail", type: "error", message: "error when connecting to server", error: e, options: options});
        }
    });
};
